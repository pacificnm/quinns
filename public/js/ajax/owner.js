				...
				success: function(data) {
					...
					if(data['noResult'] == null){
						$(data).each(function(index) {
							$(".ownerAutoComplete ul").append("<li class=\"name\" style=\" cursor:pointer\" onclick=\" document.getElementById('owner').value = '" + data[index].name+ "';\">"+BoldSearchTerm(data[index].name , data[index].name) +  " " + data[index].street + "</li>");
						});
					} else {
