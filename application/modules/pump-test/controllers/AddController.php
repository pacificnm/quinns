    ...
    public function indexAction ()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($this->getRequest()
                ...
                $pumpTestId = $pumpTestModel->create($id, $owner, 
                        $requirements, $source, $depth, $diameter, $equipment, 
                        $level, $vent, $seal, $popOffValve, $color, $taste, 
                        $odor, $employee, $remarks, $date);
                
                $this->redirect(
                        '/pump-test/add/data/id/' . $pumpTestId . '/start/' .
                                 $startTime);
            } else {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($this->getRequest()
                ...
                for ($i = $startTime * 60; $i <= $endTime * 60; $i += 15) {
                    ...
                    $id = $pumpFlowModel->create($pumpTestId, $flow, $static, 
                            $time);
                ...
                $this->redirect(
                        '/pump-test/view/index/id/' . $pumpTestId .
                                 '/msg/pump-test-add');
            } else {
                $form->highlightErrorElements();
            }
