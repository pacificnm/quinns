...
class Application_Model_DbTable_Error extends Zend_Db_Table_Abstract
{
    ...
    public function verifyTable() {
    	...
    	try{
    		$result = $db->describeTable('error'); //throws exception
    		if (empty($result)) return self::NO_TABLE;
    	}catch(Exception $e){
    		return self::NO_TABLE;
    	}
    	$result = $db->fetchRow('SELECT * FROM controller WHERE error_id = 1');
